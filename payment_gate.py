"""Pay-per-query gate for social-intel's paid tools — on-chain USDC micropayments
on Solana, fronted by a daily free tier. Same model as gov-contracts-mcp, but the
price VARIES per call (trending_topics/topic_sentiment/viral_content/
community_pulse $0.01, brand_mentions $0.02, daily_brief $5), so the price is
threaded through the gate and bound into the payment memo.
...
logger = logging.getLogger("social.pay")
...
def _price_for(tool: str, default: float = 0.01) -> float:
    """Resolve a tool's price from config.TOOL_PRICES (falls back to `default`)."""
    try:
        return float(config.TOOL_PRICES.get(tool, default))
